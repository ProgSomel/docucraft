...
export default function Sidebar({ docs }) {
  ...
  // Compute matched docs directly during render
  ...
  // Compute root nodes and grouped non-roots directly
  const rootNodes = matchedDocs?.filter((doc) => !doc.parent) || [];
  const nonRootNodesGrouped = Object.groupBy(
    matchedDocs?.filter((doc) => doc.parent) || [],
    ({ parent }) => parent,
  );

  const nonRootsKeys = Reflect.ownKeys(nonRootNodesGrouped);
  nonRootsKeys.forEach((key) => {
    const foundInRoots = rootNodes.find((root) => root.id === key);
    if (!foundInRoots) {
      const foundInDocs = docs.find((doc) => doc.id === key);
      rootNodes.push(foundInDocs);
    }
  });
          ...
          <ul role="list" className="border-l border-transparent">
            {rootNodes.map((root) => (
              <li key={root.id}>
                ...
                {nonRootNodesGrouped[root.id] && (
                  <ul role="list" className="border-l border-transparent">
                    {nonRootNodesGrouped[root.id].map((subRoot) => (
                      <li key={subRoot.id}>
                        <Link
